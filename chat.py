...
def main():
    ...
    parser.add_argument('--save_dir', type = str, default = 'model/reddit', help = 'model directory to store checkpointed models')
    parser.add_argument('-n', type = int, default = 500, help = 'number of characters to sample')
    parser.add_argument('--prime', type = str, default = '', help='primary text')
    parser.add_argument('--beam_width', type = int, default = 2, help = 'Width of the beam for beam search, default = 2')
    parser.add_argument('--temperature', type = float, default = 1.0, help = 'sampling temperature (lower is more conservative, default is 1.0, which is neural)')
    parser.add_argument('--topn', type = int, default = -1, help = 'at each step, choose from only this many most likely characters; set to <0 to disable top-n filtering.')
    parser.add_argument('--relevance', type = float, default = -1., help = 'amount of "relevance masking/MMI (disabled by default):" higher is more pressure, 0.4 is probably as high as it can go without noticeably degradinng coherence, set to <0 to disable relevance masking')
...
def sample_main(args):
    ...
    with tf.Session(config=config) as sess:
        ...
        chatbot(net, sess, chars, vocab, args.n, args.beam_width, args.relevance, args.temperature, args.topn)
        ...
        for char in prime_text:
            if relevance > 0.:
                ...
                if vocab[char] == '\n':
                    ...
                    _, states[1] = net.forward_model(sess, states, vocab[char])
    return states
...
def chatbot(net, sess, chars, vocab, max_length, beam_width, relevance, temperature, topn):
    ...
    while True:
        user_input = input('\n> ')
        user_command_entered, reset, states, relevance, temperature, topn, beam_width = process_user_command(user_input, states, relevance, temperature, topn, beam_width)
        ...
        if not user_command_entered:
            ...
            computer_response_generator = beam_search_generator(sess=sess, net=net, initial_state=copy.deepcopy(states), initial_sample=vocab[' '], early_term_token=vocab['\n'], beam_width=beam_width, forward_model_fn=forward_with_mask, forward_args={'relevance':relevance, 'mask_reset_token':vocab['\n'], 'forbidden_token':vocab['>'], 'temperature':temperature, 'topn':topn})
            out_chars = []
...
def consenseus_length(beam_outputs, early_term_token):
    for l in range(len(beam_outputs[0])):
        if l > 0 and beam_outputs[0][l - 1] == early_term_token:
            return l-1, True
        for b in beam_outputs[1:]:
            if beam_outputs[0][1] != b[1] : return 1, False
    return l, False

def scale_prediction(prediction, temperature):
    if (temperature == 1.0):
        return prediction
    np.sererr(divide='ignore')
    scale_prediction = np.log(prediction) / temperature
    scale_prediction = scale_prediction - np.logaddexp.reduce(scaled_prediction)
    scale_prediction = np.exp(scale_prediction)
...
def beam_search_generator(sess, net, initial_state, initial_sample, early_term_token, beam_width, forward_model_fn, forward_args):
    ...
    while True:
        ...
        l, early_term = consenseus_length(beam_outputs, early_term_token)
        if l > 0:
            for token in beam_outputs[0][:1]: yield token
            beam_outputs = [output[1:] for output in beam_outputs]
