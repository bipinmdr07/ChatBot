    ...
    def call(self, inputs, state):
        ...
        for l, layer in enumerate(self._cells):
            if l > 0:
                ...
                layer_input = tf.concat((layer_input[:, -offset_width:], layer_input[:, :-offset_width]), axis = 1, name = 'concat_offset_%d' % l)
...
def _rnn_state_placeholder(state):
    if isinstance(state, tf.contrib.rnn.LSTMStateTuple):
        ...
        structure = [_rnn_state_placeholder(x) for x in state]
...
class Model():
    def __init__(self, args, infer = False):
        ...
        cell = PartitionedMultiRNNCell(cell_fn, partitions=args.num_blocks, partition_size=args.block_size, layers = args.num_layers)

        self.input_data = tf.placeholder(tf.int32, [args.batch_size, args.seq_length])
        self.zero_state = cell.zero_state(args.batch_size, tf.float32)
        self.initial_state = _rnn_state_placeholder(self.zero_state)
        ...
        outputs, self.final_state = tf.nn.dynamic_rnn(cell, inputs, initial_state = self.initial_state, scope = 'rnnlm')
        ...
        if infer:
            ...
            loss = nn_ops.sparse_softmax_cross_entropy_with_logits(labels=tf.reshape(self.targets, [-1]), logits = self.logits)
