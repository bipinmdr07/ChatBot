...
class TextLoader():
    def __init__(self, data_dir, batch_size, seq_length):
        ...
        if self._preprocess_required(vocab_file, sizes_file, self.tensor_file_template, self.input_file_count):
            ...
            for i, filename in enumerate(self.input_files):
                t1 = time.time()
                print("Preprocessing file {} / {} ({})...".format(i + 1, len(self.input_files), filename), end = '', flush = True)
            ...
            print("Processed input data: {:,d} characters loaded ({:.1f} seconds)".format(self.tensor.size, time.time() - t0))
        else:
    ...
    def _preprocess(self, input_file, tensor_file):
        if input_file.endswith(".bz2"):
            ...
            file_reference = io.open(input_file, mode = 'rt')
    ...
    def _load_preprocessed(self, tensor_index):
        ...
        if self.tensor_batch_counts[tensor_index] != self.num_batches:
            print("Error in batch size! Expected: {:, d}; found: {:, d}".format(self.tensor_batch_counts[tensor_index], self.num_batches))
