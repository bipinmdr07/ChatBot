...
def main():
    ...
    parser.add_argument('--data_dir', type = str, default = 'data/reddit-data/reddit', help = 'data directory containing input.txt')
    parser.add_argument('--save_dir', type = str, default = 'model/new_save', help = 'directory for checkpointed models (load from here if one is already present)')
    parser.add_argument('--block_size', type = int, default = 2048, help = 'number of cells per block')
    parser.add_argument('--num_blocks', type = int, default = 3, help = 'number of blocks per layer')
    parser.add_argument('--num_layers', type = int, default = 3, help = 'number of layers')
    parser.add_argument('--model', type = str, default = 'gru', help = 'rnn, gru, lstm or nas')
    parser.add_argument('--batch_size', type = int, default = 40, help = 'minibatch size')
    parser.add_argument('--seq_length', type = int, default = 40, help = 'RNN sequence length')
    parser.add_argument('--num_epochs', type = int, default = 50, help = 'number of epochs')
    parser.add_argument('--save_early', type = int, default = 5000, help = 'save frequency')
    parser.add_argument('--grad_clip', type = float, default = 5., help = 'clip gradients at this value')
    parser.add_argument('--learning_rate', type = float, default = 1e-5, help = 'learning rate')
    parser.add_argument('--decay_rate', type = float, default = 0.975, help = 'how much to decay the learning rate')
    parser.add_argument('--decay_steps', type = int, default = 100000, help = 'how often to decay the learning rate')
    parser.add_argument('--set_learning_rate', type = float, default = -1, help = 'reset learning rate to this value (if greater than zero)')
...
def train(args):
    ...
    if not os.path.exists(args.save_dir):
        ...
        if ckpt and ckpt.model_checkpoint_path:
            with open(os.path.join(args.save_dir, 'config.pkl'), 'rb') as f:
                ...
                print("Found a previous checkpoint. Overwritin model description arguments to:")
                print(" model: {}, block_size: {}, num_blocks: {}, num_layers: {}".format(saved_args.model, saved_args.block_size, saved_args.num_blocks, saved_args.num_layers))
                load_model = True
    ...
    with tf.Session(config = config) as sess:
        ...
        if load_model:
            print("Resuming from global epoch fraction {:.3f}, total trained time: {}, learning rate: {}".format(global_epoch_fraction, datatime.timedelta(seconds = float(gloabal_seconds_elapsed)). sess.run(model.lr)))
        ...
        try:
            for e in range(*epoch_range):
                ...
                for b in range(*batch_range):
                    ...
                    print("{:, d} / {:, d} (epoch: {:.3f} / {}), loss {:.3f} (avg {:.3f}), {:.3f}s".format(b, batch_range[1], e + b / batch_range[1], epoch_range[1], train_loss, avg_loss, elapsed))
                
                    if (e * batch_range[1] + b + 1) % args.save_every == 0 or (e == epoch_range[1] - 1 and b == batch_range[1] - 1):
                        save_model(sess, saver, model, args.save_dir, global_step, data_loader.total_batch_count, global_seconds_elapsed)
...
def save_model(sess, saver, model, save_dir, global_step, steps_per_epoch, global_seconds_elapsed):
    ...
    print("Saving model to {} (epoch fraction {:.3f})...".format(checkpoint_path, global_epoch_fraction), end = '', flush = True)
